...
public class DeleteUser extends JDialog {
    ...
    private void onOK() {
        ...
        try {
            ...
            if (rs.next()) {
                // we now know there is a user with that username
                prepStatement = con.prepareStatement("DELETE FROM ACCOUNTS WHERE username=?");
                prepStatement.setString(1, textField1.getText().trim());
                ...
                JOptionPane.showMessageDialog(null, "No user with that username exists");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
